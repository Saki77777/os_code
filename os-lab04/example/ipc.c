...
// 修正：删除重复声明，因为这些通常是上面 *_key 或 *_num 的别名
// int buff_number;
// int pput_number;
// int cget_number;

// 信号量键值定义 (如果 *_h 是 *_key 的别名，只保留一个)
key_t prod_key; // 用 prod_key 替代 prod_h
key_t pmtx_key; // 用 pmtx_key 替代 pmtx_h
key_t cons_key; // 用 cons_key 替代 cons_h
key_t cmtx_key; // 用 cmtx_key 替代 cmtx_h
// key_t pput_h; // 如果 pput_key 已经定义，这个可以删除
// key_t buff_h; // 如果 buff_key 已经定义，这个可以删除
// key_t cget_h; // 如果 cget_key 已经定义，这个可以删除
...
// IPC flags and initial values 定义
int sem_val;
int sem_flg;
int shm_flg;
